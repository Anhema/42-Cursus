...
/*   Updated: 2022/08/09 16:43:42 by aherrero         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	continue_validate_map(t_validate *map, char **lines, char *file)
{
	...
	while (++map->i < map->y)
	{
		...
		{
			if (lines[map->i][j] != ' ' && lines[map->i][j]
			!= '0' && lines[map->i][j] != '1' && lines[map->i][j]
			!= 'N' && lines[map->i][j] != 'S' && lines[map->i][j]
			!= 'E' && lines[map->i][j] != 'W')
				print_map_error("Invalic character in map", file, lines);
			if (lines[map->i][j] == '0'
			|| lines[map->i][j] == 'N' || lines[map->i][j] == 'S'
